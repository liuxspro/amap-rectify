...
from .transform import wgs2gcj # 来自 Geohey 的转换算法 https://github.com/GeoHey-Team/qgis-geohey-toolbox
...
def get_tile(x: int, y: int, z: int, mapid: str) -> Image:
    """
    同步获取瓦片，支持高精度坐标转换（保持向后兼容性）
    """
    map_name = amap_name[mapid]
    # 小于 9 级时 没有明显的偏移 直接使用 GCJ02 坐标系的瓦片
    if z <= 9:
        return get_tile_gcj(x, y, z, mapid)
    ...
    for i, ax in enumerate(range(x_min, x_max + 1)):
        for j, ay in enumerate(range(y_min, y_max + 1)):
            tile = get_tile_gcj(ax, ay, z, mapid)
            if tile:
                composite.paste(tile, (i * 256, j * 256))
...
if __name__ == "__main__":
    # Example usage
    x1, y1, z1 = 13523, 6531, 14
    get_tile(x1, y1, z1, "vec")
